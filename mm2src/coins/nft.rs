...
use crate::nft::nft_structs::{build_nft_with_empty_meta, BuildNftFields, ClearNftDbReq, NftCommon, NftCtx, NftInfo, NftLists, NftsTransferHistoryLists, NftTransferCommon, PhishingDomainReq, PhishingDomainRes, RefreshMetadataReq, SpamContractReq, SpamContractRes, TransferMeta, TransferStatus, UriMeta};
...
/// Returns `GetNftInfoError` variants for issues like invalid requests, transport failures,
/// database errors, and spam protection errors.
pub async fn get_nft_transfers(ctx: MmArc, req: NftTransfersReq) -> MmResult<Vec<NftsTransferHistoryLists>, GetNftInfoError> {
    let db_ids = find_unique_nft_account_ids(&ctx, req.chains.clone())
        .await
        .map_to_mm(GetNftInfoError::Internal)?;

    let get_nft_transfers =
        |db_id: String, chains: Vec<Chain>| -> Pin<Box<dyn Future<Output=MmResult<NftsTransferHistoryLists, GetNftInfoError>> + Send>> {
            let ctx = ctx.clone();
            let req = req.clone();
            ...
            let res = async move {
                ...
                Ok(NftsTransferHistoryLists { transfer_history, pubkey: db_id })
            };
